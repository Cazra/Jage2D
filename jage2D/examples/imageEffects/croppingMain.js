...
function TestApp(canvas,id) {
    ...
    self.render = function(pen) {
        ...
        if(self.imageLoader.isLoading) {
            ...
            // once the finish has just finished loading, replace it with the cropped version.
            if(self.testImg.justFinishedLoading) {
                self.testImg.justFinishedLoading = false;
                self.testImg = JagePen.cropImage(self.testImg, 300, 200, 400,600);
            }
            
            // draw our cropped test image.
            pen.drawImage(self.testImg,50,50);
